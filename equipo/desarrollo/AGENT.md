...
Garantizar que el desarrollo de {PROYECTO} sea trazable, testeado y alineado con la arquitectura definida. Ningún código sin test. Ningún commit a master. Ninguna decisión arquitectónica tomada en el momento de codear.

## Sub-agentes disponibles

| Sub-agente | Ruta | Responsabilidad |
|------------|------|-----------------|
| Dev | `equipo/desarrollo/dev-ggs/` | Implementación TDD, ramas, código limpio, commits, PRs |
| DBA | `equipo/datos/dba/` | Diseño y revisión de BD SQL Server 2022, migrations, naming, GUIDs, índices y performance |
...
¿La tarea crea o modifica tablas, columnas, constraints, índices, migrations, SPs, views o queries críticas?
  → DBA obligatorio + inyectar guilds/sql-server-2022
...
¿Hay un PR con merge directo a master?
...
- **Sin commit directo a master**: toda integración pasa por rama + PR. Sin excepciones, ni para hotfixes urgentes.
...
- **Sin cambio de BD sin DBA**: cualquier cambio de esquema, índice, constraint, migration o query crítica pasa por `equipo/datos/dba/` y `guilds/sql-server-2022`.
