...
## Política GGS — implementación sobre Tasks/Bugs

- El Dev trabaja principalmente sobre Tasks o Bugs con AB# vinculado, no sobre User Stories abiertas de forma genérica.
- Toda Task o Bug debe estar estimado en Story Points antes de entrar al sprint.
- Una Task se cierra cuando el PR está aprobado y mergeado.
- Un Bug se cierra cuando el fix está mergeado y validado.
- El merge de la última Task no cierra automáticamente la User Story: queda pendiente deploy + sign-off funcional del Analista Funcional.
- Si falta parent User Story o AC, no se codea. Si la Task/Bug no tiene estimación en Story Points, tampoco entra a desarrollo: se devuelve al tablero.
...
¿AC está en español (DADO/CUANDO/ENTONCES)?
  → Sí → proceder con implementación
  → No → pedir al analista que reescriba en español
...
## AC en Gherkin Español

Los Criterios de Aceptación deben seguir la regla centralizada en `reglas/gherkin/AGENT.md`.
...
- **Marca de trazabilidad en cambios de agente**: si el código lo modifica un agente, agregar `GGS-TRACE` en el archivo o bloque lógico tocado para que el reviewer sepa qué agente intervino.

## Trazabilidad GGS-TRACE

Todo cambio de código hecho por este agente debe dejar una marca comentada mínima y buscable.

Formato:

```text
GGS-TRACE: actor=agent:equipo/desarrollo/dev-ggs; workItem=AB#{ID}; reason={motivo breve}; date={YYYY-MM-DD}
```

Ejemplos:

```ts
// GGS-TRACE: actor=agent:equipo/desarrollo/dev-ggs; workItem=AB#142; reason=implementa validación de alta; date=2026-04-17
```

```csharp
// GGS-TRACE: actor=agent:equipo/desarrollo/dev-ggs; workItem=AB#151; reason=corrige regla de CUIT; date=2026-04-17
```

Reglas:
- Usar una marca por archivo creado o por bloque lógico modificado.
- No marcar cada línea.
- No agregar `Co-Authored-By` ni atribución IA en commits.
- Si el cambio es manual, usar `actor=person:{rol-o-equipo}` solo si el proyecto lo pide.
- Si el archivo no admite comentarios sin romper formato, registrar la autoría en el PR y no forzar la marca.
...
Formato obligatorio: `{tipo}/AB{ID}-{descripcion-corta-en-kebab-case}`
...
Ejemplos válidos:
```
feat/AB142-login-con-google
fix/AB151-precio-negativo-checkout
refactor/AB188-extraer-servicio-notificaciones
test/AB155-cobertura-casos-borde
chore/AB112-actualizar-dependencias
```

Reglas:
- El work item `AB#` de Task o Bug es obligatorio. Sin Task/Bug no hay rama.
