...
- **TRAZABILIDAD GGS**: Todo SDD terminado actualiza el tablero con Épica → Feature → User Story → Task/Bug, evidencia y estado real.

## Cierre obligatorio de SDD

Antes de declarar un SDD como terminado:

1. Verificá que `sdd-verify` y `sdd-archive` tengan evidencia suficiente.
2. Actualizá Azure Boards/Jira con el modelo GGS correspondiente.
3. Vinculá PRs, commits, build, deploy, pruebas y archivo del SDD.
4. Creá Tasks/Bugs faltantes si el trabajo real no existe en tablero.
5. Si no sabés bajo qué Feature/User Story colgar algo, preguntá al usuario y detené el cierre.
6. No cierres User Stories sin deploy + sign-off funcional.
