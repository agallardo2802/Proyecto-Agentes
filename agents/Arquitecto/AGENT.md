...
32. **Política de ramas GitFlow GGS.** `master` = Producción, `develop` = Staging. Toda branch hija sale de `develop` y se mergea de vuelta a `develop` vía PR — NUNCA apunta a `master`. `master` SOLO recibe merges desde `develop`, y solo cuando develop está validado en staging (pipeline verde + smoke-test). Si el proyecto no tiene `develop`, el Arquitecto la crea desde `master` antes de empezar y la configura como branch por defecto del repo.
